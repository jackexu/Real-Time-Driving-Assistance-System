...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    while True:
      ...
      screen = cv2.resize(ImageGrab.grab(bbox=(10, 10, 50, 40)), (800,450))
      image_np = cv2.cvtColor(np.asarray(screen), cv2.COLOR_BGR2RGB)


      # image_np = cv2.cvtColor(np.asarray(cap), cv2.COLOR_BGR2RGB)
